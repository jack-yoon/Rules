...
function modifyAppConfig() {
    ...
    try {
        if (!!$.response.body) {
            ...
            e["config"]["homepage_feed_tab"]["tab_infos"] = e["config"]["homepage_feed_tab"]["tab_infos"].filter(e => {
            ...
            if (e["config"].hasOwnProperty("zhcnh_thread_sync")) {
                ...
                e["config"]["zhcnh_thread_sync"]["dns_ip_detector_operation_lock"] = "1";
                e["config"]["zhcnh_thread_sync"]["ZHHTTPSessionManager_setupZHHTTPHeaderField"] = "1"
            }
            t = {
...
function processUserInfo() {
    ...
    try {
        ...
        if (e && e["id"] && e.hasOwnProperty("vip_info") && e["vip_info"].hasOwnProperty("is_vip")) {
            const r = {
                id: e["id"],
                is_vip: e["vip_info"]["is_vip"] ? e["vip_info"]["is_vip"] !== undefined : false
            };
            $.logger.debug(`当前用户id：${e["id"]}，是否为VIP：${e["vip_info"]["is_vip"]}`);
            $.data.write(currentUserInfoKey, r);
            if ($.data.read("zhihu_settings_fake_vip") !== false && e["vip_info"]["is_vip"] === false) {
...
function manageBlackUser() {
    ...
    if ($.request.method === "GET") {
        try {
            ...
            let e = JSON.parse($.response.body);
            if (!!e["data"]) {
                $.logger.debug(`本次滑动获取的黑名单信息：${JSON.stringify(e["data"])}`);
                e["data"].forEach(e => {
                    if (e["name"] !== "[已重置]") {
                        n[e["name"]] = e["id"]
                    }
                ...
                if (e["paging"]["is_end"] === true) {
                    $.notification.post(`同步黑名单数据成功！当前黑名单共${Object.keys(n).length - defaultAnswerBlockedUsers.length}人。\n脚本内置黑名单${defaultAnswerBlockedUsers.length}人。`);
        ...
        try {
            ...
            if (e.hasOwnProperty("name") && e.hasOwnProperty("id")) {
                $.logger.debug(`当前需要加入黑名单的用户Id：${e["id"]}，用户名：${e["name"]}`);
                if (e["id"]) {
                    n[e["name"]] = e["id"];
                    $.data.write(blockedUsersKey, n, r.id);
                    $.logger.debug(`${e["name"]}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(n)}`);
        ...
        try {
            let e = JSON.parse($.response.body);
            if (e.success) {
                let t = $.request.url.match(/^https?:\/\/api\.zhihu\.com\/settings\/blocked_users\/([0-9a-zA-Z]*)/)[1];
                if (t) {
                    ...
                    for (let e in n) {
                        if (n[e] === t) {
                            delete n[e];
                            $.data.write(blockedUsersKey, n, r.id);
                            $.logger.debug(`${e}移出脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(n)}`);
...
function autoInsertBlackList() {
    ...
    try {
        ...
        if (t.name && t.id && t["is_blocking"] === true) {
            ...
            if (!e[t.name]) {
                $.logger.debug(`当前需要加入黑名单的用户Id：${t["id"]}，用户名：${t["name"]}`);
                e[t["name"]] = t["id"];
                $.data.write(blockedUsersKey, e, r.id);
                $.logger.debug(`${t["name"]}写入脚本黑名单成功，当前脚本黑名单数据：${JSON.stringify(e)}`);
...
function removeMoments() {
    let r = null;
    try {
        let e = JSON.parse($.response.body.replace(/(\w+"+\s?):\s?(\d{15,})/g, '$1:"$2"'));
        const n = getUserInfo();
        ...
        t = e.data.filter(e => {
            const t = c && e.target && e.target["origin_pin"] && e.target["origin_pin"].author && typeof i[e.target["origin_pin"].author.name] != "undefined";
...
async function _checkPaidContentByCloud(e, n) {
    ...
    if (!t) {
        ...
        await $.http.post({
            ...
            $.logger.debug(`云端探测结果<${typeof r.body}>\n${JSON.stringify(r.body)}`);
...
async function _checkPaidContentByLocal(n, o) {
    ...
    function r(t) {
        return new Promise(r => {
            const e = n[t];
            if (!e || e === "" || !e.startsWith("https://www.zhihu.com/appview/v2/answer")) {
                r("")
            } else {
                $.http.get({
                    url: n[t],
                    ...
                    const t = e.body;
                    if ((t.indexOf("查看完整内容") >= 0 || t.indexOf("查看全部章节") >= 0) && t.indexOf("paid") >= 0) {
                        r("付费内容")
                    } else if (t.indexOf("ad-link-card") >= 0 || t.indexOf("xg.zhihu.com") >= 0 || t.indexOf("营销平台") >= 0) {
                        r("营销推广")
...
async function removeRecommend() {
    ...
    try {
        ...
        const s = o => {
            ...
            const t = o["card_type"] === "slot_event_card" || o["card_type"] === "slot_video_event_card" || o.hasOwnProperty("ad") || o["brief"] && o["brief"].indexOf("slot_card") >= 0 || o["extra"] && o["extra"]["type"] === "Training";
            const r = t !== true && e.search(/"(type|style)+"\s?:\s?"(drama|zvideo|Video|BIG_IMAGE)+"/i) >= 0;
            const n = r && p;
                ...
                for (let n = 0; n < u.length; n++) {
                    if (e.search(u[n]) >= 0) {
                        if ($.isDebug) {
                            ...
                            let r = "";
                            try {
                                r = o["common_card"]["feed_content"]["title"]["action"]["intent_url"]
                            } catch { }
                            $.logger.debug(`匹配关键字：\n${u[n]}\n标题：\n${e}\n内容：\n${t}`);
                            $.notification.debug(scriptName, `关键字：${u[n]}`, `${e}\n${t}`, r)
                        }
                        c = true;
                        break
                    }
            ...
            let d;
            try {
                d = c !== true && y && f && o["common_card"]["feed_content"]["source_line"]["elements"][1]["text"]["panel_text"] in f
            } catch {
                d = false
            }
            return !(t || s || l || n || c || d)
        ...
        if (r === true) {
            ...
            for (let t = 0; t < n.length; t++) {
                if (n[t]) {
                    try {
                        let e = n[t]["common_card"]["feed_content"]["title"]["action"]["intent_url"];
                        e = e.replace(/^https:\/\/zhihu\.com\/question\/\d+\/answer\//, "https://www.zhihu.com/appview/v2/answer/");
                        r.push(e)
                    } catch {
                        r.push("")
                    }
            ...
            if ($.data.read("zhihu_settings_blocked_users", false) === true) {
                ...
                if (typeof t.root != "undefined") {
                    ...
                    if (o === true) {
                        ...
                        t.root.author.avatar_url = "https://picx.zhimg.com/v2-abed1a8c04700ba7d72b45195223e0ff_xll.jpg"
                    }
                }
                if (typeof t.data != "undefined") {
                    t.data.forEach(e => {
                        ...
                        if (o === true || i === true) {
                            ...
                            } else if (!o && n && !i && $.request.url.indexOf("child_comment") > 0) {
                                $.notification.debug(`屏蔽黑名单用户“${t}”的子评论。`)
                            } else if (o && !i && $.request.url.indexOf("child_comment") > 0) {
                                $.notification.debug(`屏蔽黑名单用户“${t}”回复“${r}”的子评论。`)
                            } else if (o && i && $.request.url.indexOf("child_comment") > 0) {
                                $.notification.debug(`屏蔽黑名单用户“${t}”回复黑名单用户“${r}”的子评论。`)
                            }
                            l[e.id] = t;
                            if (o) {
                                e.is_delete = true;
                                ...
                                e.author.name = "[黑名单用户]";
                                e.author.avatar_url = "https://picx.zhimg.com/v2-abed1a8c04700ba7d72b45195223e0ff_xll.jpg"
                            }
                            if (i) {
                                e["reply_to_author"].name = "[黑名单用户]";
                                e["reply_to_author"].exposed_medal = {};
                                e["reply_to_author"].avatar_url = "https://picx.zhimg.com/v2-abed1a8c04700ba7d72b45195223e0ff_xll.jpg"
                            }
                        }
                        if (e.child_comments) {
                            let i = [];
                            e.child_comments.forEach(e => {
                                const t = e.author.name;
                                const r = typeof e["reply_to_author"] != "undefined" ? e["reply_to_author"].name : "";
                                ...
                                if (n || o) {
                                    ...
                                    if (o === true) {
                                        ...
                                        e["reply_to_author"].exposed_medal = {};
                                        e["reply_to_author"].avatar_url = "https://picx.zhimg.com/v2-abed1a8c04700ba7d72b45195223e0ff_xll.jpg";
                                        $.notification.debug(`隐藏“${t}”回复黑名单用户“${r}”的名称与头像。`);
            ...
            if (n.length > 0) {
                ...
                e.data.forEach(e => {
                    ...
                    if (t === false) {
                        ...
                        $.notification.debug(`屏蔽翻页过程中出现的黑名单用户回答Id:${e.id}`)
                    }
                });
    ...
    try {
        if (!!$.response.body) {
            ...
            e.data.data.forEach(e => {
                ...
                if ($.data.read("zhihu_settings_blocked_users", false) === false || t === false) {
                    n.push(e)
                }
            });
            e.data.data = n;
...
function MagicJS(e = "MagicJS", t = "INFO") {
    const n = () => {
        ...
        const i = typeof $environment !== "undefined" && typeof $environment["stash-build"] !== "undefined";
        const s = n || e || o || i;
...
function MagicNotification(i, s, a, l) {
    ...
    function t(e = i, t = "", r = "", n = "") {
        ...
        a.notify(`title:${e}\nsubTitle:${t}\nbody:${r}\noptions:${typeof n === "object" ? JSON.stringify(n) : n}`);
        if (s.isSurge) {
...
function MagicData(s, a) {
    ...
    const i = t => {
        if (typeof t === "string") {
            ...
            try {
                e = JSON.parse(t);
                const r = typeof e;
                if (r !== "object" || e instanceof Array || r === "bool" || e === null) {
                    e = {}
                }
            } catch { }
            return e
    ...
    const r = (e, t = null) => {
        ...
        a.debug(`READ ALL SESSIONS [${e}] <${typeof r}>\n${JSON.stringify(r, null, 4)}`);
        return r
    };
    const n = (e, t = null) => {
        let r = {};
...
function MagicHttp(c, d) {
    ...
    class e {
        ...
        use(e, t, r) {
            ...
            this.handlers.push({
                fulfilled: e,
                rejected: t,
                synchronous: r && typeof r.synchronous === "boolean" ? r.synchronous : false,
    ...
    function o(e) {
        ...
        if (!!r.params) {
            if (!c.isNode) {
                let e = Object.keys(r.params).map(e => {
    ...
    const u = (e, t) => {
        ...
        if (c.isSurgeLike) {
            ...
            if (r["method"] !== "GET" && n && n.indexOf("application/json") >= 0 && r.body instanceof Array) {
    ...
    const n = n => {
        return Object.keys(n).reduce((e, t) => {
            const r = t.split("-").map(e => e[0].toUpperCase() + e.slice(1)).join("-");
            e[r] = n[t];
            return e
        }, {})
    };
